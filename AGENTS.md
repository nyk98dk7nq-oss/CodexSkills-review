...
- `output/reviews/`: 実行時刻別フォルダ、結果記入済みチェックリスト、Markdown サマリー
...
- Skill を変更した場合は Skill Creator の `quick_validate.py` で対象 Skill を検証する。
- Python スクリプトを変更した場合は、リポジトリルートで `py -3 test_all.py` を実行する。
