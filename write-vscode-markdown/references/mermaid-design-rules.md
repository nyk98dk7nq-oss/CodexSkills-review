...
コードフェンスの言語識別子は小文字の`mermaid`とし、図の宣言は上の表に記載した大文字・小文字をそのまま使用する。例えば`flowchart LR`と`sequenceDiagram`は有効だが、`Flowchart lr`と`sequencediagram`は使用しない。バリデーターとPDF変換器はどちらもこの大文字・小文字を区別する。
